...
const sections = [
  [["Canvas et images décoratives", "Canvas and decorative images"], [["Assembler privilégie le canvas : cliquer sélectionne, déplacer positionne et les poignées redimensionnent. Un clic dans le vide masque sélection et contrôles sans modifier grille, guides ou zone de sécurité. La barre contextuelle conserve hauteur et largeur précises ; ••• regroupe positions, espacement, alignement et centrage. La sidebar conserve Guides et Tailles minimales.", "Build prioritizes the canvas: click to select, drag to position and use handles to resize. Clicking empty space hides selection and controls without changing the grid, guides or clear space. The contextual bar retains precise height and width; ••• contains positions, spacing, alignment and centering. The sidebar retains Guides and Minimum sizes."], ["Les images du guide se déplacent hors page et se redimensionnent librement. Rotation par poignée ou valeur, opacité et quatre commandes de superposition restent contextuelles. Le débordement est visible en édition et rogné aux limites du format dans les exports. Ces propriétés sont sauvegardées et annulables.", "Guide images move beyond the page and resize freely. Handle or numeric rotation, opacity and four stacking commands remain contextual. Overflow is visible while editing and clipped to the page in exports. These properties persist and support undo."], ["La hiérarchie tient sur une seule page : espacements et spécimens s’adaptent au format sans changer les styles globaux. Les associations montrent une coche verte À FAIRE, une croix rouge À ÉVITER ou un état Autorisée distinct. Les cartes JPEG ne montrent ni ratio technique ni bouton individuel de cadrage ; utiliser le réglage partagé. Les infobulles utilisent une couche globale au-dessus de l’interface.", "Hierarchy fits on one page: spacing and specimens adapt to the format without changing global styles. Pairings show a green DO check, a red AVOID cross or a distinct Allowed state. JPEG cards show neither technical ratios nor individual framing buttons; use shared framing. Tooltips use a global layer above the interface."]]],
  [["Édition visuelle et livraison", "Visual editing and delivery"], [["Versions du logo s’organise par variante, avec vue d’ensemble et sections repliables Original, Couleurs simples, Multicolores, Dégradés, JPEG et Dimensions. Les fonctions avancées restent disponibles dans les disclosures.", "Logo versions are organized by variant, with an overview and collapsible Original, Solid colors, Multicolor, Gradients, JPEG and Dimensions sections. Advanced capabilities remain available in disclosures."], ["Un clic sur le canvas ouvre une toolbar contextuelle. Double-clic pour saisir le texte. Police, graisse disponible, taille, couleur et alignement sont près du texte ; image : Remplacer, cadrage, zoom ; logo : variante, couleur disponible, application à toute la page. Les options secondaires sont dans •••.", "A canvas click opens contextual controls. Double-click edits text. Font, available weight, size, color and alignment stay near text; images offer Replace, crop and zoom; logos offer real variants, colors and page-wide application. Secondary options stay under •••."], ["Chaque texte hérite de son rôle global. Une exception locale affiche Réinitialiser au style. Les déplacements ne créent pas d’override typographique. Accent est indépendant de Légende et possède police, graisse, taille, leading et tracking ; la hiérarchie s’ajuste sur une seule page.", "Each text inherits its global role. Local exceptions expose Reset to style. Moving text creates no typographic override. Accent is independent from Caption and has font, weight, size, leading and tracking; hierarchy fits on a single page."], ["Les nouveaux projets utilisent SVG/PNG/JPEG pour WEB et PDF vectoriel pour PRINT ; Générer aussi les bitmaps PRINT est facultatif. Les formats digitaux restent WEB. Fond transparent implique PNG, fond coloré implique JPEG ; les anciens choix sont préservés et les options avancées peuvent activer les deux.", "New projects use SVG/PNG/JPEG for WEB and vector PDF for PRINT; Also generate PRINT bitmaps is optional. Digital formats remain WEB. Transparent backgrounds imply PNG, colored backgrounds JPEG; legacy choices are retained and advanced options can enable both."], ["Les SVG indépendants proposent Texte éditable ou Texte vectorisé et des groupes nommés BACKGROUND, DECORATION, IMAGES, LOGOS, TEXT, répétés si nécessaire pour conserver les superpositions. Le PDF utilise les mêmes métriques et tracés TTF/OTF que le canvas.", "Independent SVG pages offer Editable text or Outlined text and named BACKGROUND, DECORATION, IMAGES, LOGOS and TEXT groups, repeated when required to preserve stacking. PDF uses the same TTF/OTF glyph metrics and outlines as the canvas."], ["Les recommandations ciblées ouvrent un chat persistant en mémoire de session. Le contexte et la variante sont connus dès le premier message ; les questions suivantes affinent la proposition sans modifier le projet avant Appliquer.", "Focused recommendations open a chat retained in session memory. Context and variant are known from the first message; follow-ups refine the proposal without changing the project before Apply."]]],
  ...
  [
    ...
    [
      ...
      [
        "Le ZIP utilise (NOM) LOGOKIT/LOGOS/Variante/WEB/PNG, SVG, JPEG ; PRINT contient les PDF vectoriels ; les bitmaps 300 DPI sont une option avancée ; CAS D’USAGE sépare chaque nom de format puis PNG et JPEG à 72 DPI. Les dimensions standards ne deviennent jamais des cas d’usage. CLEARSPACE/Variante sépare aussi SVG, PNG et PDF. Le TXT contient chaque couleur, son nom, HEX, RVB et CMJN approximatif sans profil ICC.",
        "ZIP structure is (NAME) LOGOKIT/LOGOS/Variant/WEB/PNG, SVG, JPEG; PRINT contains vector PDFs; 300 DPI bitmaps are an advanced option; CAS D’USAGE separates each named use case, then PNG and JPEG at 72 DPI. Standard dimensions never become use cases. CLEARSPACE/Variant also separates SVG, PNG and PDF. TXT includes every palette color, name, HEX, RGB and approximate CMYK without an ICC profile.",
      ],
      [
  ...
  [
    ...
    [
      ...
      [
        "Pour une variante générée, le canvas porte la construction. Sélectionner un élément affiche ses contrôles précis contextuels. La sidebar conserve Guides et Tailles minimales. Le zoom reste au-dessus du canvas ; Mode focus masque les panneaux et Afficher les panneaux les restaure.",
        "For generated variants, construction happens on the canvas. Selecting an element shows contextual precision controls. The sidebar retains Guides and Minimum sizes. Zoom stays above the canvas; Focus mode hides panels and Show panels restores them.",
      ],
      [
...
sections.unshift([
  ...
  [
    ...
    [
      "Le PDF conserve les logos et formes en vectoriel et convertit les textes TTF/OTF en tracés avec les métriques du canvas. Les grandes images sont transmises au moteur PDF par URL temporaire afin d’éviter un dépassement de pile, sans modifier le projet ni rasteriser la page. Seuls les exemples flou/lueur deviennent raster. Aucun CMJN ICC ni calque Illustrator n’est garanti.",
      "PDF retains vector logos and shapes and outlines TTF/OTF text using the canvas metrics. Large images are passed to the PDF engine through temporary URLs to prevent call-stack overflow, without changing the project or rasterizing the page. Only blur/glow examples become raster. ICC CMYK and Illustrator layers are not guaranteed.",
    ],
    [
      "Les actions IA utilisent une icône ✦ commune avec tooltip. Le modèle actif se change directement dans le chat. Le catalogue affiche uniquement les modèles compatibles images et outils, avec recherche instantanée et filtres Gratuit, Peu cher, Moyennement cher et Cher ; un ID non vérifié reste dans Avancé. Seul Appliquer modifie le projet et les clés restent séparées du .binksy.",
      "AI actions share a compact ✦ icon with a tooltip. The active model can be changed directly in the chat. The catalogue only shows models compatible with images and tools, with instant search and Free, Low cost, Mid-price and Expensive filters; an unverified ID remains under Advanced. Only Apply changes the project and keys stay outside the .binksy file.",
    ],
  ],
]);
...
sections.push([
  ...
  [
    ...
    ["Dimensions supplémentaires ajoute un format nommé, transparent (PNG) ou avec fond coloré (JPEG), dans le dossier de la variante. Le même réglage Taille du logo dans l’image est présent dans JPEG et chaque dimension exportée. Chaque aperçu reprend le ratio réel, dont 9:16 pour une Story, avec un cadrage propre au format et à la variante.", "Additional dimensions adds a named transparent PNG or colored-background JPEG format inside the variant folder. The same Logo size in image control is available in JPEG and every exported size. Each preview uses the actual ratio, including 9:16 for a Story, with framing specific to the format and logo variant."],
    ["Le guide réutilise les graphiques de zone de sécurité sans leurs textes. Les tailles minimales utilisent les valeurs sources : mm × 72 / 25,4 en PDF, px × 0,75 pour le digital. Les exemples sont répartis sur plusieurs pages si nécessaire, sans réduction des tailles. Un exemple plus grand que la page doit être revu avant export. La hiérarchie présente famille, graisse et tailles, sans interlignage ni tracking. Les pages proposent les variantes et couleurs réelles. Toutes les associations sont visibles, avec état Recommandée, Autorisée ou À éviter, sans baisse d’opacité et avec pagination.", "The guide reuses clear-space artwork without embedded labels. Minimum sizes use source values: mm × 72 / 25.4 in PDF, px × 0.75 for digital. Hierarchy shows family, weight and sizes, without leading or tracking. Pages offer real logo variants and colors. All pairs are visible as Recommended, Allowed or Avoid, at full opacity with pagination."],
