...
import { finalPalette, colorRoles } from './guideline-config.js';
...
export const GUIDE_ACTIONS = ['updatePageSettings','updateGlobalSettings','updatePageElement','updateLogoOccurrence','updateLogoOccurrences','updateTypography','updateColorAssociation','togglePage','addPageElement','removePageElement','updateGuideExports','updateAccentTypography','updatePaletteRole','addColorRole'];
...
export function applyGuideAction(p,a,scope) {
  ...
  if(!keys(a,['type','pageId','id','values','elementId','variant','colorId','occurrences','role','foreground','background','decision','enabled','elementType','name'])) throw Error('Action IA invalide.');
  const g=p.brandGuideline, page=g.pages.find(q=>q.id===(a.pageId||a.id)), v=validators(p);
  if(a.type==='updateAccentTypography') {
    patch(a.values,{enabled:bool,font:x=>x===''||g.resources.some(r=>r.id===x&&r.type==='font')});
    if('enabled' in a.values)g.accentTypography={enabled:a.values.enabled};
    if('font' in a.values){const f=g.resources.find(r=>r.id===a.values.font);g.typography.accent={...g.typography.accent,font:a.values.font,family:f?.family||'Instrument Sans',weight:f?.weight||400};}
  } else if(a.type==='addColorRole') {
    if(!str(a.name,100)||!a.name.trim())throw Error('Rôle invalide.');
    g.customColorRoles=[...new Set([...(g.customColorRoles||[]),a.name.trim()])];
  } else if(a.type==='updatePaletteRole') {
    const c=g.palette?.find(c=>c.id===a.id);
    if(!c || !Object.hasOwn(colorRoles(g),a.role))throw Error('Rôle invalide.');c.role=a.role;
  } else if(a.type==='addPageElement') {
    ...
    for(const entry of occurrences) {
      ...
      if(!el || !v.variant(variant) || (entry.colorId!=='auto' && !logoChoices(p,variant).some(c=>c.id===entry.colorId)))throw Error('Variante ou couleur de logo inexistante.');
    ...
    if(![...ROLES,'accent'].includes(a.role))throw Error('Rôle typographique invalide.');
