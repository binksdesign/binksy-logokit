...
export function palettePanel(p) {
  return `<section class="palette-panel"><h2>${t("Palette de couleurs")}</h2><p>${t("Les couleurs détectées dans le logo sont ajoutées automatiquement.")}</p><div class="palette-swatches">${p.colors.map((col) => `<button class="palette-swatch" data-edit-color="${esc(col.id)}" style="--swatch:${col.hex}" aria-label="${esc(col.name)} · ${col.hex}" title="${esc(col.name)} · ${col.hex}"><span data-no-i18n>${esc(col.name)}</span></button>`).join("")}<button class="palette-add" data-action="add-color">+ ${t("Ajouter une couleur")}</button></div></section>`;
}
...
export function workspace(
  ...
  const left =
    view === "import"
      ? projectPanel(p, projects) + imports(p, font) + palettePanel(p)
      : view === "compose"
        ? constructions(p)
        : view === "delivery"
