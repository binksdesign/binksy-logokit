  ...
  const right =
    view === "compose" && ready
      ...
      : view === "delivery"
        ...
        : view === "family"
          ? `<section><h2>${t("Votre sélection")}</h2><strong class="selected-versions" role="status"></strong><span id="selection-count" role="status"></span><button class="primary" data-view="guideline">${t("Préparer l’export")}${arrow}</button></section>${disclosure("logo-colors", "Modifier les couleurs du logo", rolePanel(p))}${disclosure("palette", "Palette de couleurs", palettePanel(p))}`
          : "";
  const next =
    view === "import"
      ? "compose"
